...
CodePress = {
	...
	keyHandler : function(evt) {
		...
		if((evt.ctrlKey || evt.metaKey) && evt.shiftKey && charCode!=90)  { // shortcuts = ctrl||appleKey+shift+key!=z(undo) 
			...
			top.setTimeout(function(){CodePress.syntaxHighlight('generic');},0);
		}
		else if(keyCode==9 || evt.tabKey) {  // snippets activation (tab)
			editor.innerHTML=editor.innerHTML+'&#9';
	...
	complete : function(trigger) {
		...
		for (var i=0; i<complete.length; i++) {
			if(complete[i].input == trigger) {
				...
				parent.setTimeout(function () { CodePress.syntaxHighlight('complete',pattern,content)},1000); // wait for char to appear on screen
			}
		}
	},
